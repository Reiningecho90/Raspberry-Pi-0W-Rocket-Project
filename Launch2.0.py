...
static_flight_alt = input("Input the level you expect to touch down at: ")
backup_flight_alt = input("Input the backup altitude in case of a failure: ")
highest_alt = 0
...
try:
    while status:
        ...
        sys.stdout.write(f'Altitude: {altitude}')
        sys.stdout.flush()
        sys.stdout.write('\n')
        sys.stdout.flush()
        sys.stdout.write(f'Pressure: {pressure}')
        sys.stdout.flush()
        sys.stdout.write('\n')
        sys.stdout.flush()
        sys.stdout.write(f'Air Temp: {temperature}')
        sys.stdout.flush()
        sys.stdout.write('\n')
        sys.stdout.flush()
        sys.stdout.write(f'Highest Altitude Reached: {highest_alt}')
        sys.stdout.flush
        ...
        if deploy:
            sys.stdout.write("Confirmation of parachute deployment via altimeter and pressure sensor, visual parachute confirmation should dictate final call of deploy status.")
            sys.stdout.flush()

        if altitude > highest_alt:
            highest_alt = altitude

        if altitude == static_flight_alt:
            sys.stdout.write("TOUCHDOWN CONFIRMED, CRAFT HAS REACHED EXPECTED TOUCHDOWN ALTITUDE")
            sys.stdout.flush()

except IOError as e:
    print("IO error detected, launch scrub.")
    print(e)
